(function(){
  const STYLE=`.ai-session-modal{position:fixed;inset:0;background:rgba(12,16,24,.62);z-index:1000;display:flex;align-items:center;justify-content:center;padding:20px}.ai-session-panel{width:min(900px,100%);max-height:90vh;overflow:auto;background:#fff;border-radius:24px;padding:28px;box-shadow:0 24px 80px rgba(0,0,0,.22)}.ai-session-head{display:flex;justify-content:space-between;gap:20px;align-items:flex-start}.ai-session-head h2{margin:4px 0 8px}.ai-session-close{border:0;background:#f1f2f5;border-radius:12px;width:40px;height:40px;font-size:22px;cursor:pointer}.ai-session-meta{color:#68707d;margin-bottom:20px}.ai-summary{font-size:16px;line-height:1.65}.ai-grid{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-top:20px}.ai-box{background:#f7f8fa;border-radius:16px;padding:18px}.ai-box h3{margin:0 0 10px;font-size:15px}.ai-box ul{margin:0;padding-left:20px;line-height:1.6}.ai-full{grid-column:1/-1}.ai-status{padding:14px 16px;border-radius:12px;background:#f3f5f7;margin:15px 0}.ai-questions{display:grid;gap:10px}.ai-q{padding:12px 14px;background:#fff;border:1px solid #e3e6ea;border-radius:12px}.ai-error{background:#fff0f0;color:#a32626}.ai-progress{height:7px;background:#e7e9ed;border-radius:99px;overflow:hidden}.ai-progress i{display:block;height:100%;width:0;background:#111;transition:width .3s}@media(max-width:700px){.ai-grid{grid-template-columns:1fr}.ai-full{grid-column:auto}}`;
  const style=document.createElement('style');style.textContent=STYLE;document.head.appendChild(style);
  const modal=document.createElement('div');modal.className='ai-session-modal';modal.hidden=true;modal.innerHTML=`<div class="ai-session-panel"><div class="ai-session-head"><div><p class="label">PABLO STUDY · IA</p><h2 id="aiTitle">Preparando tu sesión</h2><div id="aiMeta" class="ai-session-meta"></div></div><button class="ai-session-close" id="aiClose">×</button></div><div id="aiBody"></div></div>`;document.body.appendChild(modal);
  const title=document.getElementById('aiTitle'),meta=document.getElementById('aiMeta'),body=document.getElementById('aiBody');
  document.getElementById('aiClose').onclick=()=>modal.hidden=true;
  ...
  async function extractPdf(blob){
    ...
    for(let i=1;i<=maxPages;i++){
      ...
      if(pageText)text+=`\n\n[PÁGINA ${i}]\n${pageText}`;
      if(i%10===0)progress(Math.min(70,Math.round(i/maxPages*70)));
    }
  ...
  async function prepare(session){
    ...
    modal.hidden=false;title.textContent=subject.name;meta.textContent=`${new Date(session.scheduled_for).toLocaleDateString('es-ES',{weekday:'long',day:'numeric',month:'long'})} · ${session.duration_minutes} min`;
    ...
    try{
      ...
      await state.sb.from('study_sessions').update({ai_content:r,ai_generated_at:new Date().toISOString()}).eq('id',session.id).eq('user_id',state.user.id);
    ...
    }catch(e){console.error(e);body.innerHTML=`<div class="ai-status ai-error"><b>No he podido preparar la sesión.</b><br>${esc2(e.message||e)}</div><p>Si el error indica que falta GEMINI_API_KEY, solo hay que añadir esa clave a los Secrets de Supabase. No se mete en GitHub ni en el navegador.</p>`;}
  }
  function wire(){
    const btn=document.getElementById('startBtn');if(btn&&!btn.dataset.aiWired){btn.dataset.aiWired='1';btn.textContent='Preparar sesión';btn.onclick=()=>{const s=state.sessions.filter(x=>x.status==='planned').sort((a,b)=>new Date(a.scheduled_for)-new Date(b.scheduled_for))[0];if(s)prepare(s);else toast('No hay ninguna sesión preparada.');};}
    const next=document.getElementById('nextSession');if(next&&!next.dataset.aiObserver){next.dataset.aiObserver='1';const obs=new MutationObserver(()=>{const b=document.getElementById('startBtn');if(b&&!b.dataset.aiWired){b.dataset.aiWired='1';b.textContent='Preparar sesión';b.onclick=()=>{const s=state.sessions.filter(x=>x.status==='planned').sort((a,b)=>new Date(a.scheduled_for)-new Date(b.scheduled_for))[0];if(s)prepare(s);};}});obs.observe(next,{childList:true,subtree:true});}
  }
